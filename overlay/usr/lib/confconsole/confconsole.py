...
class TurnkeyConsole:
    ...
    def _get_advmenu(self):
        ...
        items.append(("PageKite", "Make HTTP & SSH servers accessible online"))
    ...
    def _adv_pagekite(self):
	text = "PageKite makes local websites or SSH servers publicly\n"
	text+= "accessible in mere seconds, and works with any computer\n"
	text+= "and any Internet connection. Sign up for an account at\n"
	text+= "https://pagekite.net/signup/."
	
	keys, values = pkconf.read()

	fields = []
	i = 0
	for key in keys:
		fields.append((key, values[i], 40, 200))
		i=i+1

	retcode, input = self.console.form("PageKite settings", text, fields)
	
	if retcode is self.OK:
		pkconf.saveandreload(keys, input)
